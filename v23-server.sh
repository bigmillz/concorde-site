...
#   ./v23-server.sh install
#   ./v23-server.sh install --obfs --down 110 --up 14
#   ./v23-server.sh install --domain vpn.example.com --email me@example.com
#   ./v23-server.sh uninstall
#
set -euo pipefail
...
v23 — Hysteria 2 server bootstrap for Ubuntu
  ...
  v23-server.sh install [options]
  v23-server.sh uninstall [--yes]
...
do_uninstall() {
  ...
  ok "v23 removed (ufw rules left in place)"
  exit 0
}
...
validate_reality_port() {
  ...
  if [[ -n "$busy" ]] && (( REALITY_PORT == busy )); then
   ...
   already holds. To put reality on 443, move the others first:
     ./v23-server.sh install --port 8443 --reality-port 443
...
XRTEST
    ...
    if [[ -n "$XR_SEEN" ]]; then
      ...
      die  "re-run with:  ./v23-server.sh reality --reality-sni dl.google.com"
    fi
...
do_reality() {
  [[ $EUID -eq 0 ]] || die "must run as root"
  [[ -r "$META_FILE" ]] || die "no ${META_FILE} — run './v23-server.sh install' first"
...
# ---------------------------------------------------------------- preflight --
say "Preflight"
[[ $EUID -eq 0 ]] || die "must run as root (sudo ./v23-server.sh install)"
...
[[ -r "$META" ]] || { echo "v23 is not installed"; exit 1; }
. "$META"
  ...
  obfs)
    ...
    case "${2:-}" in
      ...
      off)
        ...
        ;;
      *) echo "usage: v23 obfs on|off"; exit 1 ;;
    esac
    ;;
  rotate)
    need_root
...
echo
say "Paste this block into ConcordeVPN's first-run screen (again later: v23 show)"
"${CLI_PATH}" show
